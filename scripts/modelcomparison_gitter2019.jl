...
fitdat_a_time = @df Tdat_a PrimaryDryFit(:t[:T .<-20u"°C"], :T[:T .< -20u"°C"] .|> u"K", maximum(Tdat_a.T)|>u"K", (6.0u"hr", 12.0u"hr"))
# plot(fitdat_a_time)
...
pl_q = qplotrf(sol_a, ordering=[2,1,3])
...
pl_1a = plot(pl_T, pl_q, pl_mw, pl_p; size=(600,600), layout=@layout([a{0.6h}; b; c; d]), link=:x)
end
...
plot!(xticks=0:2:10)
pl_1b = plot(pl_T, pl_q, pl_mw, pl_p; size=(600,600), layout=@layout([a{0.6h}; b; c; d]), link=:x)
end
...
begin
pl_T = blankplot_hrC(xwiden=false)
exptfplot!(T1dat_c.t, T1dat_c.T, T2dat_c.T, nmarks=40, label=[L"$T_\textrm{f}$, Gitter et al., 2019" " "])
...
pl_mw = plot(xunit=u"hr", xwiden=false, ylabel="MW\nPower\n")
plot!(MWdat_c.t, MWdat_c.P, c=:red, label="")
plot!(ylim=(0, 105))
plot!(xlabel="", xticks=(0:2:10, []), bottom_margin=-10Plots.px)
pl_p = plot(xlabel="Time", xunit=u"hr", xwiden=false, ylabel=L"$p_\textrm{ch}$"*"\n")
plot!(pdat_c.t, pdat_c.pch, c=3, label="")
plot!(ylims=(0, 17), unitformat=:square)
pl_1c = plot(pl_T, pl_q, pl_mw, pl_p; size=(600,700), layout=@layout([a{0.6h}; b; c; d]), link=:x)
end
