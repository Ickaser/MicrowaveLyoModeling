...
trans_Kshf = as(ConstPhysProp, (TVScale(10.0u"W/m^2/K") ∘ TVShift(0.001) ∘ TVLogistic(),))
...
err_nls_a_time = NonlinearFunction{true, SciMLBase.FullSpecialize}(nls_g, resid_prototype=zeros(num_errs(fitdat_a_time)))
err_nls_b = NonlinearFunction{true, SciMLBase.FullSpecialize}(nls_g, resid_prototype=zeros(num_errs(fitdat_b)))
err_nls_b_time = NonlinearFunction{true, SciMLBase.FullSpecialize}(nls_g, resid_prototype=zeros(num_errs(fitdat_b_time)))
err_nls_c = NonlinearFunction{true, SciMLBase.FullSpecialize}(nls_g, resid_prototype=zeros(num_errs(fitdat_c)))
err_nls_c_time = NonlinearFunction{true, SciMLBase.FullSpecialize}(nls_g, resid_prototype=zeros(num_errs(fitdat_c_time)))
